...
public:
    ...
    friend std::ostream& operator <<(std::ostream& out_stream, const  Puzzle_8& tmp) {
        std::ofstream myfile("Myfile.txt",std::ios_base::app);
        system("cls");
        ...
        //===============================================
        myfile << "MOVE: " << tmp.what_move << "   # " << tmp.node_number << "     h(n)=" << tmp.h_n << "    g(n)= " << tmp.depth << std::endl;
        myfile << "+------+-------+-------+" << std::endl;
        for (int i = 0; i < 3; i++) {
            myfile << "|   ";
            for (int j = 0; j < 3; j++) {
